...
# 获取旧数据目�?read -p "请输入旧 Vaultwarden 数据目录的完整路�? " OLD_DATA_DIR

# 验证路径
if [ ! -d "$OLD_DATA_DIR" ]; then
    log_error "目录不存�? $OLD_DATA_DIR"
    exit 1
fi
...
# 获取旧容器名�?read -p "请输入旧 Vaultwarden 容器名称 [vaultwarden]: " OLD_CONTAINER
OLD_CONTAINER="${OLD_CONTAINER:-vaultwarden}"

# 检查旧容器状�?if docker ps -a --format '{{.Names}}' | grep -q "^${OLD_CONTAINER}$"; then
    OLD_CONTAINER_RUNNING=$(docker inspect -f '{{.State.Running}}' "$OLD_CONTAINER" 2>/dev/null || echo "false")
    log_info "找到旧容�? $OLD_CONTAINER (运行�? $OLD_CONTAINER_RUNNING)"
else
    log_warn "未找到容�? $OLD_CONTAINER"
...
echo "=========================================="
echo ""
echo "源数据目�? $OLD_DATA_DIR"
echo "目标数据目录: $NEW_DATA_DIR"
echo "旧容�? $OLD_CONTAINER"
echo ""
log_warn "迁移步骤:"
echo "  1. 停止�?Vaultwarden 容器"
echo "  2. 备份旧数�?
echo "  3. 复制数据到新目录"
echo "  4. 初始化新配置"
echo "  5. 启动�?Vaultwarden"
echo "  6. 验证迁移结果"
echo ""
log_warn "注意: 迁移期间服务将短暂不可用"
echo ""

read -p "确认开始迁�? (输入 YES 继续): " confirm
if [ "$confirm" != "YES" ]; then
    echo "已取消迁�?
    exit 0
fi
...
# 复制数据�?if command -v sqlite3 &> /dev/null; then
    sqlite3 "$OLD_DATA_DIR/db.sqlite3" ".backup '$NEW_DATA_DIR/db.sqlite3'"
else
    cp "$OLD_DATA_DIR/db.sqlite3" "$NEW_DATA_DIR/"
fi
# 不复�?WAL 文件，让新实例重新创�?rm -f "$NEW_DATA_DIR/db.sqlite3-wal" "$NEW_DATA_DIR/db.sqlite3-shm"
...
# 设置权限
chown -R 1000:1000 "$NEW_DATA_DIR" 2>/dev/null || log_warn "无法修改权限，请确保 uid 1000 有写入权�?
...
# Step 5: 启动新容�?log_step "5/6 启动�?Vaultwarden..."

# 提示用户配置域名
echo ""
log_warn "请先编辑 .env 文件配置域名�?SMTP:"
echo "  nano .env"
echo ""
echo "必须配置:"
...
if docker compose ps | grep -q "running"; then
    ...
    # 检查健康状�?    CONTAINER_PORT=$(docker compose port vaultwarden 80 2>/dev/null | cut -d: -f2 || echo "")
...
echo "=========================================="
echo ""
echo "迁移前备�? backups/${BACKUP_NAME}.tar.gz"
echo ""
echo "下一�?"
echo "  1. 访问你的 Vaultwarden 域名，验证登录正�?
echo "  2. 检查密码、附件是否完�?
echo "  3. 如一切正常，可以删除旧容�?"
